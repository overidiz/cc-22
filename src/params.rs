...
pub struct Cc22Params {
    ...
    // The whole signal chain has exactly two EQs: one before the modules and one
    // after. (Their serialization IDs are kept legacy — `global_pre_eq_*` / `eq_*`
    // — so projects saved before this simplification still load.)
    #[nested(group = "Pre EQ")]
...
impl_eq_param_refs!(PostEqParams);
...
mod tests {
    ...
    /// The simplified EQ system has exactly two banks: Pre (legacy
    /// `global_pre_eq_*` IDs) and Post (legacy `eq_*` IDs). No per-module banks.
    #[test]
    fn eq_parameter_ids_match_the_two_bank_schema() {
        let params = Cc22Params::default();
        let all_ids = params
            .param_map()
            .into_iter()
        ...
        // The removed per-module banks must no longer exist.
        for bank in [
            "character_pre_eq",
            "character_post_eq",
            "movement_pre_eq",
            ...
            let probe = format!("{bank}_b1_gain");
            assert!(
                !all_ids.contains(&probe),
                "module EQ bank should be gone: {probe}"
            );
        }
    }
    ...
    fn expected_eq_parameter_ids() -> BTreeSet<String> {
        ...
        // Pre EQ keeps the legacy global_pre_eq_* IDs (backward-compat).
        let bank = "global_pre_eq";
        ...
        // Post EQ keeps the legacy eq_* IDs (backward-compat).
        for id in [
            "eq_mode",
            "eq_bypass",
            "eq_band1_enabled",
